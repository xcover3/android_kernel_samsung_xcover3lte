...
void __kernel *osa_pages_alloc(uint32_t nr)
{
	...
	if (size <= KMEM_ALLOC_MAX_SIZE) {
		 ...
		 * PageReserved is set.
		 */
		phys = (void *)(unsigned long)__virt_to_phys((unsigned long)ret);
		for (i = 0; i < nr; i++) {
			 ...
			 void **phys)
{
	...
	if (is_cached) {
		...
		alloc_phys_addr = (void *)(unsigned long)__virt_to_phys((unsigned long)alloc_virt_addr);
	} else {
		alloc_virt_addr =
...
void *osa_virt_to_phys_ex(void *virt_addr, bool *cacheable)
{
	...
	if (virt_addr < high_memory &&
		...
		*cacheable = true;
		return (void *)(unsigned long)__virt_to_phys((ulong_t)virt_addr);
	} else {
