...
#define FTS_GESTURE_ADDRESS 0xd3
#define FTS_GESTURE_LENGTH	4
static short gesture_pointnum = 0;
//static unsigned short coordinae_x[150] = {0};
//static unsigned short coordinae_y[150] = {0};
#endif
...
static ssize_t virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	return sprintf(buf,"%s:%s:%d:%d:%d:%d\n"
		,__stringify(EV_KEY), __stringify(KEY_HOMEPAGE),pdata ->virtualkeys[0],pdata ->virtualkeys[1],pdata ->virtualkeys[2],pdata ->virtualkeys[3]);
}
...
#ifdef FTS_GESTURE
static int ft5x0x_gesture_data(void)
{
	int ret = -1;
	unsigned char buf[FTS_GESTURE_POINTS * 2] = {0};
    int gesture_id = 0;
    int gesture_keycode = 0;

    buf[0] = FTS_GESTURE_ADDRESS;
    ret = ft5x0x_i2c_rxdata(buf, 8);
	if (ret < 0) {
    ...
    {
        gesture_id = 0x24;
		pr_debug("===gesture id===", gesture_id);
        return 0;
    }

    gesture_pointnum = (short)buf[1] & 0xff;
    buf[0] = FTS_GESTURE_ADDRESS;
    gesture_id = fetch_object_sample(buf, gesture_pointnum);
    switch (gesture_id)
    {
        case GESTURE_LEFT:
            gesture_keycode = KEY_LEFT;
        case GESTURE_DOUBLECLICK:
            gesture_keycode = KEY_POWER;

        default:
            break;
    }
    if (gesture_keycode > 0)
    {
        input_report_key(g_ft5x0x_ts->input_dev, gesture_keycode, 1);
        input_sync(g_ft5x0x_ts->input_dev);
        input_report_key(g_ft5x0x_ts->input_dev, gesture_keycode, 0);
        input_sync(g_ft5x0x_ts->input_dev);
    }
    return 0;
}
...
//	u8 ft_pressure , ft_size;
	...
	event->prox_event = 0;
	//printk("ft5x0x_ts_pen_irq_work result:%x\n",result);
	if(result == 0xc0 && is_incall==1)
	{
		printk("[ft5x0x] face close 0n\n");
		event->prox_event = 1;
		event->prox_value = 1;
...
#ifdef FTS_GESTURE
    if ((g_ft5x0x_ts->fts_gesture_enable) && (g_ft5x0x_ts->fts_gesture_mode))
        ft5x0x_gesture_data();
    else
#endif
		ft5x0x_update_data();
...
static void ft5x0x_ts_suspend(struct early_suspend *handler)
{
	...
	//gpio_direction_output(g_ft5x0x_ts->platform_data->reset_gpio_number, 0);//for future use
}
...
static int ft5x0x_ts_suspend(struct device *dev)
{
	...
	pr_info("==%s==\n", __FUNCTION__);
	g_ft5x0x_ts->fts_suspend = true;
#ifdef FTS_GESTURE
    #ifdef TP_HAVE_PROX
	if(is_incall == 1)
	{
		PRINT_ERR("ft5x0x_ts_suspend gesture prox is_incall");
        return 0;
	}
    #endif
    if (g_ft5x0x_ts->fts_gesture_enable)
    {
	    pdata->set_reset();
	    ft5x0x_write_reg(0xd0, 0x01); //fw open gesture
        g_ft5x0x_ts->fts_gesture_mode = true;
        return 0;
    }
#else
#ifdef TP_HAVE_PROX
	if(is_incall == 1)
	{
		PRINT_ERR("ft5x0x_ts_suspend prox is_incall");
	}else {
	ret = ft5x0x_write_reg(FT5X0X_REG_PMODE, PMODE_HIBERNATE);
	if(ret){
		PRINT_ERR("==ft5x0x_ts_suspend==  ft5x0x_write_reg fail\n");
...
static int ft5x0x_ts_resume(struct device *dev)
{
	...
	pr_info("==%s==\n", __FUNCTION__);
	g_ft5x0x_ts->fts_suspend = false;
#ifdef FTS_GESTURE
    #ifdef TP_HAVE_PROX
	if(is_incall == 1)
	{
		PRINT_INFO("ft5x0x_ts_resume gesture prox is_incall");
		ft5x0x_prox_ctl(1);
	    return 0;
	}
    #endif
    ...
    {
	    pdata->set_reset();
	    ft5x0x_write_reg(0xd0, 0x00); //fw close gesture
        g_ft5x0x_ts->fts_gesture_mode = false;
	    return 0;
    }
#else
	pdata->set_power(1);
	...
	if(is_incall == 1)
	{
		PRINT_INFO("ft5x0x_ts_resume prox is_incall");
		ft5x0x_prox_ctl(1);
	}
#endif
	enable_irq(this_client->irq);
...
static ssize_t store_proximity_sensor(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	PRINT_ERR("qiao_store_proximity_sensor buf=%d,size=%d,on_off=%d\n", *buf, size, on_off);
	if(buf != NULL && size != 0)
	{
		if (0 == on_off)
		{
...
static DEVICE_ATTR(proximity, 0777, show_proximity_sensor, store_proximity_sensor);
#endif
...
if (!g_ft5x0x_ts->v_tsp) {
		g_ft5x0x_ts->v_tsp = regulator_get(&client->dev, "focaltech,v_tsp");
	...
	if (!state && on) {
		...
		regulator_enable(g_ft5x0x_ts->v_tsp);
	} else if (!on && state) {
		state = 0;
...
static void ft5x0x_touch_reset(void)
{
	...
	gpio_direction_output(pdata->reset_gpio_number, 1);
	msleep(100);
	printk(KERN_INFO "ft5x0x_touch reset done.\n");
	gpio_free(pdata->reset_gpio_number);
out:
	return;
}
...
static struct ft5x0x_ts_platform_data *ft5x0x_ts_parse_dt(struct device *dev)
{
	...
	match = of_match_device(focaltech_dt_ids, dev);
	if (!match) {
       dev_err(dev, "Compatible mismatch\n");
		goto fail;
	}

	ret = of_property_read_u32(np, "focaltech,abs-x-max", &pdata->abs_x_max);
	if (ret){
		dev_err(dev, "Failed to get property max_x from node(%d)\n", ret);
		goto fail;
	}
	ret = of_property_read_u32(np, "focaltech,abs-y-max", &pdata->abs_y_max);
	if (ret){
	...
	ret = of_property_read_u32_array(np, "focaltech,virtualkeys", &pdata->virtualkeys,4);
	if(ret){
...
if (ft_rw_iic_drv_init(client) < 0)
{
...
#endif
	ft5x0x_ts->fts_suspend = false;
#ifdef FTS_GESTURE
	ft5x0x_ts->fts_gesture_enable = true;
	ft5x0x_ts->fts_gesture_mode = false;
    init_para(pdata->abs_x_max, pdata->abs_y_max, 50, 0, 0);
	//ft5x0x_write_reg(0xd0, 0x01); //fw open gesture
#endif
#ifdef TP_HAVE_PROX
	...
	if(device_create_file(firmware_cmd_dev, &dev_attr_proximity) < 0) // /sys/class/mtk-tpd/device/proximity
	{
...
static struct i2c_driver ft5x0x_ts_driver = {
	...
	.driver	= {
		...
		.owner	= THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(focaltech_dt_ids),
#endif
#ifdef CONFIG_PM_RUNTIME
