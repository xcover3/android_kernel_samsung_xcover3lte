...
int pm886_buck_set_suspend_mode(struct regulator_dev *rdev, unsigned int mode)
{
	struct pm886_buck_info *info = rdev_get_drvdata(rdev);
	u8 val;
	int ret;
...
static unsigned int pm886_buck_get_optimum_mode(struct regulator_dev *rdev,
...
static int pm886_buck_get_current_limit(struct regulator_dev *rdev)
{
	struct pm886_buck_info *info = rdev_get_drvdata(rdev);
	if (!info)
		return 0;
...
static int pm886_buck_set_suspend_voltage(struct regulator_dev *rdev, int uv)
{
	int ret, sel;
	struct pm886_buck_info *info = rdev_get_drvdata(rdev);
	...
	ret = pm886_buck_set_suspend_mode(rdev, REGULATOR_MODE_IDLE);
	return ret;
}
...
static struct regulator_ops pm886_volt_buck_ops = {
	...
	.get_current_limit = pm886_buck_get_current_limit,
	.get_optimum_mode = pm886_buck_get_optimum_mode,
	.set_suspend_mode = pm886_buck_set_suspend_mode,
	.set_suspend_voltage = pm886_buck_set_suspend_voltage,
};

/* The array is indexed by id(PM886_ID_XXX) */
static struct pm886_buck_info pm886_buck_configs[] = {
...
#define PM886_BUCK_OF_MATCH(comp, label) \
	{ \
		.compatible = comp, \
		.data = &pm886_buck_configs[PM886_ID_##label], \
	}
static const struct of_device_id pm886_bucks_of_match[] = {
...
static int pm886_buck_probe(struct platform_device *pdev)
{
	...
	const struct pm886_buck_info *const_info;
	struct pm886_buck_info *info;
	int ret;

	match = of_match_device(pm886_bucks_of_match, &pdev->dev);
	if (match) {
...
static int pm886_buck_remove(struct platform_device *pdev)
{
...
static struct platform_driver pm886_buck_driver = {
	.driver		= {
		.name	= "88pm886-buck",
		.owner	= THIS_MODULE,
		.of_match_table = of_match_ptr(pm886_bucks_of_match),
	},
	.probe		= pm886_buck_probe,
	.remove		= pm886_buck_remove,
};

static int pm886_buck_init(void)
{
	return platform_driver_register(&pm886_buck_driver);
}
subsys_initcall(pm886_buck_init);

static void pm886_buck_exit(void)
{
	platform_driver_unregister(&pm886_buck_driver);
}
module_exit(pm886_buck_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Yi Zhang<yizhang@marvell.com>");
MODULE_DESCRIPTION("Buck for Marvell 88PM886 PMIC");
MODULE_ALIAS("platform:88pm886-buck");
