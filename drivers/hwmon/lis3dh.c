...
#ifdef CONFIG_OF
static int sensor_set_power(struct device *dev, int on)
{
...
#ifdef CONFIG_OF
	err = sensor_set_power(&acc->client->dev, 0);
#else
	if (acc->pdata->power_off)
		err = acc->pdata->power_off(0);
#endif
	if (err < 0) {
		dev_err(&acc->client->dev,
				"warning: power_off failed: %d\n", err);
	}
	acc->hw_initialized = 0;
}
...
#ifdef CONFIG_OF
	err = sensor_set_power(&acc->client->dev, 1);
#else
...
static int lis3dh_acc_enable(struct lis3dh_acc_data *acc)
{
	...
	pr_info("sprd-gsensor: -- %s -- success!\n", __func__);
	return 0;
}
				...
				unsigned long arg)
{
	...
	case LIS3DH_ACC_IOCTL_GET_CHIP_ID:
	{
		...
		if (copy_to_user(argp, devinfo, sizeof(devinfo))) {
			pr_err("%s error in copy_to_user(IOCTL_GET_CHIP_ID)\n",
					__func__);
			return -EINVAL;
		}
		break;
...
static struct lis3dh_acc_platform_data *lis3dh_acc_parse_dt(struct device *dev)
{
	...
	if (!pdata) {
		dev_err(dev,
			"Could not allocate struct lis3dh_acc_platform_data");
		return NULL;
	}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if ((tempvalue & 0x00FF) == WHOAMI_LIS3DH_ACC) {
		...
		acc->client = NULL;
		pr_debug("I2C driver not registered! Device unknown 0x%x\n",
				tempvalue);
		goto err_destoyworkqueue2;
	}
...
static int lis3dh_acc_pm_resume(struct device *dev)
{
...
static int lis3dh_acc_pm_suspend(struct device *dev)
{
	struct lis3dh_acc_data *acc = dev_get_drvdata(dev);
	dev_dbg(dev, "###%s###\n", __func__);
...
static const struct dev_pm_ops lis3dh_pm_ops = {
	.suspend = lis3dh_acc_pm_suspend,
	.resume  = lis3dh_acc_pm_resume,
};
