...
struct elan_epl_data {
	...
	int intr_pin;
	int (*power)(struct device *dev, bool status);
...
//txbyte: I2C bus transmit byte(s). Single byte(0X01)
//transmit only slave address.
//data: setting value.
//
...
static int elan_sensor_psensor_enable(struct elan_epl_data *epld)
{
	...
	ret = elan_sensor_I2C_Write(client, REG_9, W_SINGLE_BYTE,
		0x02, EPL_INT_DISABLE);

	regdata = EPL_SENSING_8_TIME | EPL_PS_MODE | EPL_H_GAIN;
	regdata = regdata |
	    (PS_INTERRUPT_MODE ? EPL_C_SENSING_MODE : EPL_S_SENSING_MODE);
	ret = elan_sensor_I2C_Write(client, REG_0,
		W_SINGLE_BYTE, 0X02, regdata);

	regdata = PS_INTT<<4 | EPL_PST_1_TIME | EPL_10BIT_ADC;
	ret = elan_sensor_I2C_Write(client, REG_1, W_SINGLE_BYTE,
		0X02, regdata);

	set_psensor_intr_threshold(P_SENSOR_LTHD, P_SENSOR_HTHD);

	ret = elan_sensor_I2C_Write(client, REG_7, W_SINGLE_BYTE,
		0X02, EPL_C_RESET);
	ret = elan_sensor_I2C_Write(client, REG_7, W_SINGLE_BYTE,
		0x02, EPL_C_START_RUN);
...
static int elan_sensor_lsensor_enable(struct elan_epl_data *epld)
{
	...
	regdata = EPL_S_SENSING_MODE | EPL_SENSING_8_TIME |
	    EPL_ALS_MODE | EPL_AUTO_GAIN;
	...
	ret = elan_sensor_I2C_Write(client, REG_10, W_SINGLE_BYTE,
		0x02, EPL_GO_MID);
	ret = elan_sensor_I2C_Write(client, REG_11, W_SINGLE_BYTE,
		0x02, EPL_GO_LOW);
	ret = elan_sensor_I2C_Write(client, REG_7, W_SINGLE_BYTE,
		0X02, EPL_C_RESET);
	ret = elan_sensor_I2C_Write(client, REG_7, W_SINGLE_BYTE,
...
//Variable "value" 0 and 1 to represent which distance from psensor
//to target(human's face..etc).
...
static void elan_epl_ps_poll_rawdata(void)
{
	...
	elan_sensor_I2C_Read(client);
	gRawData.ps_ch1_raw = (gRawData.raw_bytes[1] << 8) |
	    gRawData.raw_bytes[0];

	elan_sensor_I2C_Write(epld->client, REG_7, W_SINGLE_BYTE,
		0x02, EPL_DATA_UNLOCK);
...
static void epl_sensor_irq_do_work(struct work_struct *work)
{
	...
	elan_sensor_I2C_Write(client, REG_9, W_SINGLE_BYTE,
		0x02, EPL_INT_ACTIVE_LOW);
	elan_sensor_I2C_Write(client, REG_7, W_SINGLE_BYTE,
		0x02, EPL_DATA_UNLOCK);
	enable_irq(epld->irq);
}
...
static void polling_do_work(struct work_struct *work)
{
	...
	if (isAlsOnly || isInterleaving) {
	    ...
	    elan_sensor_I2C_Write(client, REG_9, W_SINGLE_BYTE,
		    0x02, EPL_INT_DISABLE);
	    elan_sensor_I2C_Write(client, REG_0, W_SINGLE_BYTE,
		    0X02, EPL_S_SENSING_MODE);
	}
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	elan_sensor_I2C_Write(epld->client, REG_16, R_TWO_BYTE,
		0x01, 0x00);
...
#if 0
	epld->als_input_dev->name = "TSL_light_sensor";
	epld->als_input_dev->id.bustype = BUS_I2C;
	epld->als_input_dev->id.vendor = 0x0089;
	epld->als_input_dev->id.product = 0x2771;
	epld->als_input_dev->id.version = 3;
	epld->als_input_dev->phys = "tsl2x7x";
#endif
...
#if 0
	/* epld->ps_input_dev->id = ; */
	epld->ps_input_dev->id.bustype = BUS_I2C;
	epld->ps_input_dev->id.vendor = 0x0089;
	epld->ps_input_dev->id.product = 0x2771;
	epld->ps_input_dev->id.version = 3;
	epld->ps_input_dev->phys = "tsl2x7x";
#endif
	set_bit(EV_ABS, epld->ps_input_dev->evbit);
#if 1
	set_bit(ABS_DISTANCE, epld->ps_input_dev->absbit); /* Proximity */
#endif
	input_set_abs_params(epld->ps_input_dev, ABS_DISTANCE, 0, 1, 0, 0);
#if 1
	input_set_drvdata(epld->ps_input_dev, epld->client);
#endif
...
static int setup_interrupt(struct elan_epl_data *epld)
{
	...
	if (err) {
		...
		s3c_gpio_setpull(S5PV210_GPH0(1),
			S3C_GPIO_PULL_NONE);
	}
#endif
	epld->irq = client->irq;
	err = request_irq(epld->irq, elan_sensor_irq_handler,
		IRQF_TRIGGER_FALLING, client->dev.driver->name, epld);
	if (err < 0) {
...
static int board_epl2182_power(struct device *dev, bool status)
{
	...
	if (status) {
		result = regulator_enable(v_sensor);
...
static int elan_sensor_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (epl_data->enable_pflag == 0)	{
		elan_sensor_I2C_Write(client, REG_7,
			W_SINGLE_BYTE, 0x02, EPL_C_P_DOWN);
		cancel_delayed_work(&polling_work);
	}
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	epld->power = board_epl2182_power;
	epld->power(&client->dev, 1);
	...
	sensor_dev = platform_device_register_simple("elan_alsps", -1,
		NULL, 0);
