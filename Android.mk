...
#below script update relays on scripts/pxa1936_config.sh
ifeq ($(TARGET_SOC),pxa1936)
	$(KERNEL_SCRIPT_UPDATE) --set-val CONFIG_NR_CPUS 8
	$(KERNEL_SCRIPT_UPDATE) -d CONFIG_NO_HOTPLUG_POLICY
	$(KERNEL_SCRIPT_UPDATE) -d CPU_HOTPLUG_POL_STANDALONE
	$(KERNEL_SCRIPT_UPDATE) -d CONFIG_CPU_IDLE
	$(KERNEL_SCRIPT_UPDATE) -d CONFIG_CPU_IDLE_MMP_V8
	$(KERNEL_SCRIPT_UPDATE) -e CONFIG_ARM_MMP_BL_CPUFREQ
	$(KERNEL_SCRIPT_UPDATE) -d CONFIG_ARM_MMP_SMP_CPUFREQ
	$(KERNEL_SCRIPT_UPDATE) -d CONFIG_PXA1928_THERMAL
	$(KERNEL_SCRIPT_UPDATE) -d CONFIG_HELAN2_THERMAL
	$(KERNEL_SCRIPT_UPDATE) -e CONFIG_PXA1936_CLK
	$(KERNEL_SCRIPT_UPDATE) -d SCHED_AUTOGROUP
	$(KERNEL_SCRIPT_UPDATE) -e SCHED_MC
	$(KERNEL_SCRIPT_UPDATE) -e DISABLE_CPU_SCHED_DOMAIN_BALANCE
	$(KERNEL_SCRIPT_UPDATE) -e SCHED_HMP
	$(KERNEL_SCRIPT_UPDATE) -d SCHED_HMP_PRIO_FILTER
	$(KERNEL_SCRIPT_UPDATE) --set-str HMP_FAST_CPU_MASK 4-7
	$(KERNEL_SCRIPT_UPDATE) --set-str HMP_SLOW_CPU_MASK 0-3
	$(KERNEL_SCRIPT_UPDATE) -e HMP_VARIABLE_SCALE
	$(KERNEL_SCRIPT_UPDATE) -e HMP_FREQUENCY_INVARIANT_SCALE
	$(KERNEL_SCRIPT_UPDATE) -e SCHED_HMP_LITTLE_PACKING
endif
